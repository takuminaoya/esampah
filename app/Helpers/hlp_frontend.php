...
function displayResult($checker, $tipe)
{
    ...
    if (isNameSet($checker)) {
        if (isNameSet($tipe)) {
            $gen = '<div class="alert alert-' . $_GET[$tipe]  . '" role="alert">' . textTransform($_GET[$checker]) . '</div>';
        } else {
            $gen = '<div class="alert alert-success" role="alert">' . textTransform($_GET[$checker]) . '</div>';
        }
    }
