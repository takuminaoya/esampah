...
function getError($error, $name, $genereateClass = false)
{
    ...
    if ($error->has($name)) {
        if ($genereateClass == false) {
            $gen = '<div class="" style="color:indianred;">*' . $error->first($name) . '</div>';
        } else {
            $gen = $genereateClass;
        }
    }
...
function uploadFileDenganCompress($request, $namaInput, $namaFile, $destinasiFolder, $ukuranModifier = 0.5, $kualitasGambar = 100, $gunakanAspectRatio = false, $tujuan_path = "uploads/", $removeTujuanPath = false)
{
    $varFile = $request->{$namaInput};
    $cext = $varFile->extension();

    $nama = $namaFile . "." . $cext;
    $tujuan = $tujuan_path . $destinasiFolder . "/" . $nama;
    ...
    return $removeTujuanPath ? $destinasiFolder . "/" . $nama : $tujuan;
}
...
// fungsi untuk menyembunyikan huruf sepert nik dsb, 
// contoh : 5093*********
function hideNumber($text, $displayOnly = 4, $hideTextLimit = 12, $symbol = "*")
{
    $takeDisplayOnlyFromText = str_split($text, $displayOnly);
    $symbolContainers = [];

    for ($i = 0; $i < $hideTextLimit; $i++) {
        $symbolContainers[] = $symbol;
    }

    return $takeDisplayOnlyFromText[0] . implode("", $symbolContainers);
}
...
// mendapatkan list negara dengan menggunakan API
// https://restcountries.com/#rest-countries
function getListOfCountry(array $filters = [])
{

    $ctrs = Http::get("https://restcountries.com/v3.1/all?fields=name");
    $conts = [];

    if (count($filters) <= 0) {
        $arr = json_decode($ctrs, true);

        foreach ($arr as $warga) {
            $conts[] = $warga['name']['common'];
        }
...
{
    setcookie("auth_token", "", time() + (86400 * 30), "/", NULL, 0);
    setcookie("auth_name", "", time() + (86400 * 30), "/", NULL, 0);
    setcookie("silagas", "", time() + (86400 * 30), "/", NULL, 0);
    setcookie("auth_expired", "", time() + (86400 * 30), "/", NULL, 0); // 86400 = 1 day

    unset($_COOKIE["auth_name"]);
    unset($_COOKIE["auth_token"]);
    unset($_COOKIE["silagas"]);
    unset($_COOKIE["auth_expired"]);
}

function captchaTokenManual()
{
    $random_number = rand(111111, 999999);
    $rnToHash = Hash::make($random_number);

    session()->put(["silagas_rnth" => $rnToHash]);
}

function checkNoHpAbs($hp)
{
    if ($hp == "" or $hp == null)
        return;
    $str_wo_symnols = preg_replace('/[^\p{L}\p{N}\s]/u', '', $hp);
    $str_wo_space = str_replace(" ", "", $str_wo_symnols);
    $str_arry = str_split($str_wo_space);
    $strc = [];
...
function checkNoWaKlian($banjar)
{
    $data = DB::connection("ungasan")->table("users")->where("banjar", $banjar)->where("user_type", "staff")->first();

    return ($data != null) ? $data->no_wa : 0;
}

function reformatNoHp($noHp, $forWA = true)
{
    $removeFromHp = preg_replace("/[^0-9]/", "", $noHp);

    if($forWA){
        $split_str = str_split($removeFromHp, 2);

        if (is_array($split_str) && $noHp != "") {
            switch ($split_str[0]) {
                case "08":
                    $split_by1 = str_split($removeFromHp, 1);
                    $newstring = [6, 2];

                    for ($i = 0; $i < count($split_by1); $i++) {
                        if ($i != 0) {
                            $newstring[] = $split_by1[$i];
                        }
                    }
