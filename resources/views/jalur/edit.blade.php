                        ...
                        <div class="card-body">
                            <form action="{{ route('jalur.update', $jalur->id) }}" method="post">
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control @error('nama') is-invalid @enderror" id="nama" name="nama" value="{{ $jalur->nama }}">
                                        @error('nama')
                                            ...
                                            <div class="card-body">
                                                <ul id="sortable-banjar" class="list-group">
                                                    @foreach($details as $detail)
                                                        <li class="list-group-item d-flex justify-content-between align-items-center banjar-item" data-banjar-id="{{ $detail->banjar_id }}">
                                                           
                                                            <div>
                                                                <input type="hidden" name="banjar_ids[]" value="{{ $detail->banjar_id }}">
                                                                <span class="mr-5">
                                                                ...
                                                                <div class="col-md-4 mb-2">
                                                                    <div class="form-check">
                                                                        <input class="form-check-input available-banjar" type="checkbox" value="{{ $banjar->id }}" id="available-{{ $banjar->id }}"
                                                                            {{ $details->contains('banjar_id', $banjar->id) ? 'checked' : '' }}>
...
<script>
    $(document).ready(function() {
        ...
        // Handle remove banjar button
        $(document).on("click", ".remove-banjar", function() {
            const banjarId = $(this).data("banjar-id");
            
            // Remove from sortable list
            $(`#sortable-banjar li[data-banjar-id="${banjarId}"]`).remove();
            
            // Reinitialize sortable after removing items
            initSortable();
            
            // Uncheck in modal
            $(`#available-${banjarId}`).prop("checked", false);
            
            // Show info alert if no banjars left
            if ($("#sortable-banjar li").length === 0) {
                if ($("#sortable-banjar .alert-info").length === 0) {
                    $("#sortable-banjar").append(
                        `<div class="alert alert-info mt-3">
                            <i class="fas fa-info-circle"></i> Belum ada banjar yang ditambahkan. Silakan tambahkan banjar menggunakan tombol "Tambah Banjar".
